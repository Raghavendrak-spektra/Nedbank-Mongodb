...
## Options
Option 1 : Delete the live `customer360` database and restore the complete backup so that all collections share the same backup timestamp.
Option 2 : Restore the entire `customers` collection from the 10:00 backup with `--drop`, then recreate any newer changes that users report missing.
Option 3 : Restore the 10:00 backup into a temporary collection, select the 12 deleted records by their captured IDs, insert only records still missing from the live collection, and verify that the post-backup marker remains.
